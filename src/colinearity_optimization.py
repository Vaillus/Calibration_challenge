...
class VanishingPointEstimator:
    ...
    def estimate_vanishing_point(self, flow, visualize=False, ground_truth_point=None):
        ...
        filtered_flow, weights = self.flow_filter.filter_by_norm(flow)
    ...
    def find_vanishing_point_lbfgsb(self, flow, weights=None, visualize=True, ground_truth_point=None):
        ...
        if visualize:
            ...
            result = minimize(
                lambda point: self.objective_function(point, flow, weights),
                starting_point,
                method='L-BFGS-B',
                callback=callback,
                options={'disp': False}
            ...
            # Optimization without visualization overhead
            result = minimize(
                lambda point: self.objective_function(point, flow, weights),
                starting_point,
                method='L-BFGS-B',
